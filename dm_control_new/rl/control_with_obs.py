...
class Environment(dm_env.Environment):
  ...
  def step(self, action):
    ...
    ##GET OBSERVATION HERE 101
    obs=self._task.before_step(action, self._physics)
    for _ in range(self._n_sub_steps):
      self._physics.step()
    self._task.after_step(self._physics)

    reward = self._task.get_reward(self._physics)
    # if "swimmer" in str(self._task.__class__):
    #   # print("task test", str(self.task.__class__))
    #   observation = self._task.get_observation(self._physics)
    #   print("test")
    # else:
    observation = self._task.get_observation(self._physics)
    ...
    if obs:
      for i in range(len(obs)):
        d_obs.append(obs[i].cpu().detach().numpy())

      d_obs = np.array(d_obs).reshape((18,))
      observation['abs'] = d_obs
    if self._flat_observation:
      observation = flatten_observation(observation)
    # print(observation['abs'].shape)
