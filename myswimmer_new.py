...
class Swimmer(base.Task):
  ...
  def get_optimal_action(self, physics):
    head_orientation = physics.named.data.xmat['head']
    ...
    in_frame = nose_to_target.dot(head_orientation)[:2]
    uv1 = physics.named.data.geom_xpos['target']
    uv1 = uv1/np.linalg.norm(uv1)
    uv2 = physics.named.data.geom_xpos['nose']
    uv2 = uv2/np.linalg.norm(uv2)
    dp = np.dot(uv1, uv2)
    d = (uv2 - uv1)
